...
async function getAllArticlesByTipus(req, res) {
    try {
        const articles = await Articles.find({ tipus: req.params.tipus });
        if (!articles) return res.status(404).send({ message: "No s'ha trobat cap article" });
        return res.status(200).send({ articles });
    } catch (error) {
        return res.status(500).send({ message: `Ha sorgit l'error següent ${error}` });
...
async function getAllTipus(req, res) {
    const tipus=["Fruita", "Verdura", "Carn", "Peix", "Làctics"];
